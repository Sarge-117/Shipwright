...
bool ItemUseFromInventory_IsValidItemForUse(PlayState* play) {
    ...
    if (!(CVarGetInteger(CVAR_ENHANCEMENT("ItemUseFromInventory"), 0) && (pauseCtx->state == 6) && (pauseCtx->unk_1E4 == 0) &&
    ...
    if (this->meleeWeaponState != 0 || // Swinging sword
        this->stateFlags1 & PLAYER_STATE1_LOADING           || this->stateFlags1 & PLAYER_STATE1_SWINGING_BOTTLE      ||
        this->stateFlags1 & PLAYER_STATE1_HOOKSHOT_FALLING  || this->stateFlags1 & PLAYER_STATE1_INPUT_DISABLED       ||
        this->stateFlags1 & PLAYER_STATE1_TALKING           || this->stateFlags1 & PLAYER_STATE1_DEAD                 ||
        this->stateFlags1 & PLAYER_STATE1_READY_TO_FIRE     || this->stateFlags1 & PLAYER_STATE1_GETTING_ITEM         ||
        this->stateFlags1 & PLAYER_STATE1_CARRYING_ACTOR    || this->stateFlags1 & PLAYER_STATE1_CHARGING_SPIN_ATTACK ||
        this->stateFlags1 & PLAYER_STATE1_HANGING_OFF_LEDGE || this->stateFlags1 & PLAYER_STATE1_CLIMBING_LEDGE       ||
        this->stateFlags1 & PLAYER_STATE1_JUMPING           || this->stateFlags1 & PLAYER_STATE1_FREEFALL             ||
        this->stateFlags1 & PLAYER_STATE1_FIRST_PERSON      || this->stateFlags1 & PLAYER_STATE1_CLIMBING_LADDER      ||
        this->stateFlags1 & PLAYER_STATE1_SHIELDING         || this->stateFlags1 & PLAYER_STATE1_ON_HORSE             ||
        this->stateFlags1 & PLAYER_STATE1_USING_BOOMERANG   || this->stateFlags1 & PLAYER_STATE1_BOOMERANG_THROWN     ||
        this->stateFlags1 & PLAYER_STATE1_DAMAGED           || this->stateFlags1 & PLAYER_STATE1_IN_WATER             ||
        this->stateFlags1 & PLAYER_STATE1_IN_ITEM_CS        || this->stateFlags1 & PLAYER_STATE1_IN_CUTSCENE          ||
        this->stateFlags1 & PLAYER_STATE1_FLOOR_DISABLED    ||

        this->stateFlags2 & PLAYER_STATE2_DISABLE_ROTATION_ALWAYS ||
        this->stateFlags2 & PLAYER_STATE2_GRABBED_BY_ENEMY        || this->stateFlags2 & PLAYER_STATE2_UNDERWATER      ||
        this->stateFlags2 & PLAYER_STATE2_DIVING                  || this->stateFlags2 & PLAYER_STATE2_FROZEN          ||
        this->stateFlags2 & PLAYER_STATE2_CRAWLING                || this->stateFlags2 & PLAYER_STATE2_HOPPING         ||
        this->stateFlags2 & PLAYER_STATE2_SWORD_LUNGE             || this->stateFlags2 & PLAYER_STATE2_FORCED_VOID_OUT ||

        this->stateFlags3 & PLAYER_STATE3_MIDAIR || this->stateFlags3 & PLAYER_STATE3_FLYING_WITH_HOOKSHOT) {
        return false;
    }
    ...
    if (((gSlotAgeReqs[cursorSlot] == 9) || (gSlotAgeReqs[cursorSlot] == ((void)0, gSaveContext.linkAge))) &&
        ...
        if (interfaceCtx->restrictions.ocarina == 0 && cursorSlot == SLOT_OCARINA ||
         ...
         /* interfaceCtx->restrictions.all == 0 && cursorSlot == SLOT_LENS && gSaveContext.isMagicAcquired &&
                gSaveContext.magic >= 1 ||*/
            interfaceCtx->restrictions.dinsNayrus == 0 && cursorSlot == SLOT_DINS_FIRE &&
                gSaveContext.isMagicAcquired && gSaveContext.magic >= 12 ||
            interfaceCtx->restrictions.dinsNayrus == 0 && cursorSlot == SLOT_NAYRUS_LOVE &&
                gSaveContext.isMagicAcquired && gSaveContext.magic >= 24 ||
            interfaceCtx->restrictions.farores == 0 && cursorSlot == SLOT_FARORES_WIND &&
                gSaveContext.isMagicAcquired && gSaveContext.magic >= 12) {
        ...
        if (interfaceCtx->restrictions.tradeItems == 0 &&
            ...
            (cursorSlot == SLOT_TRADE_CHILD && !(cursorItem >= ITEM_MASK_KEATON && cursorItem <= ITEM_MASK_TRUTH) &&
             !(CVarGetInteger(CVAR_ENHANCEMENT("MaskSelect"), 0)))) {
            return true;
        }
    }
    return false;
        ...
        if (pauseCtx->cursorSpecialPos == 0) {
            ...
            if (cursorItem != PAUSE_ITEM_NONE) {
                ...
                if ((pauseCtx->debugState == 0) && (pauseCtx->state == 6) && (pauseCtx->unk_1E4 == 0)) {
                    // For enhancement "Item Use From Inventory"
                    if (CVarGetInteger(CVAR_ENHANCEMENT("ItemUseFromInventory"), 0) && ItemUseFromInventory_IsValidItemForUse(play)) {
                        pauseCtx->cursorColorSet = 8;
...
void KaleidoScope_SetupItemEquip(PlayState* play, u16 item, u16 slot, s16 animX, s16 animY) {
    ...
    if ((pauseCtx->equipTargetItem == ITEM_ARROW_FIRE) || (pauseCtx->equipTargetItem == ITEM_ARROW_ICE) ||
        ...
        if (CVarGetInteger(CVAR_ENHANCEMENT("SkipArrowAnimation"), 0)) {
            ...
            Audio_PlaySoundGeneral(NA_SE_SY_SET_FIRE_ARROW + index, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
...
void KaleidoScope_UpdateItemEquip(PlayState* play) {
    ...
    if (sEquipState == 2) {
        ...
        if (D_8082A488 == 0) {
            ...
            WREG(87) = WREG(91);
            sEquipState++;
            Audio_PlaySoundGeneral(NA_SE_SY_SYNTH_MAGIC_ARROW, &gSfxDefaultPos, 4, &gSfxDefaultFreqAndVolScale, &gSfxDefaultFreqAndVolScale, &gSfxDefaultReverb);
        }
        return;
    }
